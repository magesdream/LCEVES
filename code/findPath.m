function possiablePaths = findPath(Graph, partialPath, destination, partialWeight,edge)
...
tmpPsbPaths = findPath(Graph, tmpPath, destination, partialWeight + Graph(lastNode, nextNodes(i)),edge);
possiablePaths = cat(1, possiablePaths, tmpPsbPaths);
% 自己加的判断条件
if (~isempty(possiablePaths))
    if (length(possiablePaths(:,1))>edge)
        return;
    end  
end
% disp(tmpPath)
