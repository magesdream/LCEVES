function s_route = C_RoutePlan(orginX,orginY,destinationX,destinationY)
tic
% 读入距离网络矩阵
load G_Network
load RoadNodes
...
toc
[~,dpp] = djstl_figure(startIndex,G_Network);
toc
% 重要节点的路径
mroute = dpp(DesIndex);
froute = orgin;
...
for s = 1:length(froute)
    s_route(end+1) = froute(s,1);
    s_route(end+1) = froute(s,2);
end
toc
